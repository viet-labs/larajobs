...
@section('content')
    <div class="max-w-[95%] mx-auto mt-6">
        <h1 class="font-bold text-5xl my-4">User List</h1>
            ...
            <table class="my-2 w-full">
                <thead>
                    <tr>
                        ...
                        <th class="py-3 border-2 border-slate-900">Email</th>
                        <th class="py-3 border-2 border-slate-900">Password</th>
                ...
                <tbody class="text-center">
                    @foreach ($user as $u)
                        <tr>
                            <td class="py-3 px-6 border-2 border-slate-900">{{ $u->id }}</td>
                            <td class="py-3 px-6 border-2 border-slate-900">{{ $u->name }}</td>
                            <td class="py-3 px-6 border-2 border-slate-900">{{ $u->email }}</td>
                            <td class="py-3 px-6 border-2 border-slate-900">{{ $u->password }}</td>
                            <td class="py-3 px-6 border-2 border-slate-900">
                                <a href="{{ url('edituser/' . $u->id) }}"
                                    class="p-2 font-bold text-white bg-blue-600 border-2 border-blue-600 rounded hover:text-blue-600 hover:bg-white">Edit</a>
                                <a href="{{ url('deleteuser/' . $u->id) }}"
                                    class="p-2 font-bold text-white bg-blue-600 border-2 border-blue-600 rounded hover:text-blue-600 hover:bg-white">Delete</a>
                                <a href="{{ url('showuser/' . $u->id) }}"
